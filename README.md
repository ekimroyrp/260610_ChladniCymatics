...
260610_ChladniCymatics is a WebGPU-first Three.js cymatics explorer that renders Chladni-style sand patterns on a black field. The app uses analytic plate modes to move thousands of particles toward nodal lines in real time while a frequency dial changes the audible and visual resonance.
...
- Reference frequency preset buttons that reset the sand on each jump.
- Canvas click interaction for jumping to a random preset and resetting the sand.
...
- Responsive black-background view with soft additive off-white sand particles.
...
- Click a preset Hz button to jump to a reference Chladni mode and reset the sand; clicking the active preset restarts the tone.
- Click the canvas to jump to a random preset and reset the sand.
